...
export function WrongNotesList() {
  ...
  if (!data?.items.length) {
    return <div className="rounded-lg border border-slate-200 bg-white p-4 text-sm text-slate-500">아직 오답노트가 없습니다.</div>;
  }

  return (
    <div className="space-y-3">
      {data.items.map((note) => (
        <Link key={note.id} href={`/study/questions/${note.questionId}`} className="block rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
