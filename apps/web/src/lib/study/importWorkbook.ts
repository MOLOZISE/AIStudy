...
export async function importStudyWorkbook(input: ImportStudyWorkbookInput): Promise<ImportStudyWorkbookResult> {
  ...
  try {
    ...
    // 05_정식문제은행 전체 문제로 자동 세트 생성 (재import 시 덮어쓰기)
    if (questions.size > 0) {
      const allSetExternalId = '__all__';
      const [allSet] = await db
        .insert(studyExamSets)
        .values({
          workbookId: input.workbookId,
          subjectId: input.subjectId,
          externalId: allSetExternalId,
          title: `${input.subjectName} 전체 문제`,
          description: `05_정식문제은행 전체 ${questions.size}문항`,
      ...
      // 기존 items 삭제 후 재삽입 (문제 순서 유지)
      await db.delete(studyExamSetItems).where(eq(studyExamSetItems.setId, allSet.id));

      const allItems = [...questions.entries()].map(([, questionId], index) => ({
        setId: allSet.id,
        questionId,
        position: index + 1,
        points: '1',
      ...
      await db.insert(studyExamSetItems).values(allItems);
      examSetItems += allItems.length;
    }
